...
export async function createFeedback(req, res) {
  try {
    ...
    if (!isCustomerValid(req)) return res.status(403).json({ message: "Forbidden" });
...
export async function getAllFeedbacks(req, res) {
  try {
    ...
    return res.status(200).json({ message: "All Feedbacks", result: feedbacks });
  } catch (err) {
...
export async function deleteFeedback(req, res) {
  try {
    ...
    return res.status(200).json({ message: "Feedback deleted successfully", result: deleted });
  } catch (err) {
    return res.status(500).json({ message: "Failed to delete feedback", error: err.message || err });
  }
}
...
export async function updateFeedbackStatus(req, res) {
  try {
    ...
    return res.status(200).json({ message: "Feedback status updated", result: updated });
  } catch (err) {
    return res.status(500).json({ message: "Failed to update feedback", error: err.message || err });
  }
}
